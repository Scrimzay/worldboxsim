...
function getBiomeFromTerrain(terrainType) {
    ...
    if (terrainType === 1) return BIOMES.SNOW;
    if (terrainType === 2) return BIOMES.CEMETERY;
    return null;
}
...
function getBiomeAt(x, y) {
    ...
    let redCount = 0;   // North/Snow
    let blueCount = 0;  // South/Cemetery
    ...
    if (redCount > blueCount) return BIOMES.SNOW;
    if (blueCount > redCount) return BIOMES.CEMETERY;
    
    // Fallback to y-position for horizontal map (top=snow, bottom=cemetery)
    if (y < 50) return BIOMES.SNOW;
    if (y > 50) return BIOMES.CEMETERY;
    return null; // Border
}
...
const biomeEntityImages = {
    snow: {
        right: null,  // snowentity1.png - facing right (Norscan warrior)
        left: null    // snowentity2.png - facing left
    },
    cemetery: {
        right: null,  // cemeteryentity1.png - facing right (Vampire)
        left: null    // cemeteryentity2.png - facing left
    }
};
...
function draw() {
            ...
            for (let x = 0; x < GRID_SIZE; x++) {
                ...
                if (cell === 3 || cell === 5) {
                    ...
                    if (foundDirection) {
                        ...
                        // Default direction based on tribe
    ...
    for (let y = 0; y < GRID_SIZE; y++) {
        for (let x = 0; x < GRID_SIZE; x++) {
            ...
            if (cell === 1) {
                // Red terrain - snow texture (north)
                img = biomeTerrainImages.snow.terrain;
            } else if (cell === 2) {
                // Blue terrain - cemetery texture (south)
                img = biomeTerrainImages.cemetery.terrain;
            } else if (cell === 6) {
                // Trees - biome-specific (now updates with terrain conversion)
                img = biome === BIOMES.SNOW ? biomeTerrainImages.snow.trees : biomeTerrainImages.cemetery.trees;
            } else if (cell === 7) {
                // Rocks - biome-specific (now updates with terrain conversion)
                img = biome === BIOMES.SNOW ? biomeTerrainImages.snow.rocks : biomeTerrainImages.cemetery.rocks;
            } else if (cell === 8) {
                // Hills - biome-specific (now updates with terrain conversion)
                img = biome === BIOMES.SNOW ? biomeTerrainImages.snow.hills : biomeTerrainImages.cemetery.hills;
                ...
                if (biome === BIOMES.SNOW) {
                    ...
                    // On border, use snow sprite as default
                    img = biomeEntityImages.snow[direction];
                }
            } else if (cell === 4) {
                // Border
            ...
            if (img && useImages) {
                ...
                let color = '#333';
                if (cell === 1) color = '#E0F0FF';     // Light blue/white for snow
                if (cell === 2) color = '#2C1B3D';     // Dark purple for cemetery
                if (cell === 3) color = '#4A90E2';     // Blue for Norsca entity
                if (cell === 4) color = '#0044ff';     // Border
                if (cell === 5) color = '#8B0000';     // Dark red for Sylvania entity
                if (cell === 6) {
                    // Trees: pine in snow, dead trees in cemetery
                    color = biome === BIOMES.SNOW ? '#1B4D3E' : '#4A3C2F';
                }
                if (cell === 7) {
                    // Rocks: ice in snow, gravestones in cemetery
                    color = biome === BIOMES.SNOW ? '#B0C4DE' : '#696969';
                }
                if (cell === 8) {
                    // Hills: snowy mountains in snow, fog/mist in cemetery
                    color = biome === BIOMES.SNOW ? '#C0D6E4' : '#534D56';
                }
    ...
    if (event.data instanceof ArrayBuffer) {
        ...
        if (msg.action === 'inspect_response') {
            ...
            if (msg.tribes) {
                ...
                const north = msg.tribes["1"] || { count: 0, wood: 0, stone: 0, name: "Tribe 1" };
                const south = msg.tribes["2"] || { count: 0, wood: 0, stone: 0, name: "Tribe 2" };
                
                document.getElementById('northCount').innerText = north.count;
                document.getElementById('southCount').innerText = south.count;
                document.getElementById('northWood').innerText = north.wood;
                document.getElementById('northStone').innerText = north.stone;
                document.getElementById('southWood').innerText = south.wood;
                document.getElementById('southStone').innerText = south.stone;
                document.getElementById('northTribeName').innerText = north.name || tribeNames["1"];
                document.getElementById('southTribeName').innerText = south.name || tribeNames["2"];
            } else {
